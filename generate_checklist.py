...
MONTHLY_TASKS = [
    ...
    },
    {
        ...
        "subtasks": ["（　/　）", "路徑：財管系統→工作平台→理專保管客戶物品查核結果→請經理放行"]
    },
    ...
    },
    {
        ...
        "subtasks": ["路徑：內網→電子表單→作業處→金庫鑰匙(edge)"]
    },
...
def get_weeks_in_month(year, month):
    """Get the week ranges for a given month (business days only: Mon-Fri)"""
    cal = calendar.monthcalendar(year, month)
    weeks = []
    
    for week_num, week in enumerate(cal, 1):
        # Filter only business days (Monday=0 to Friday=4)
        business_days = []
        for day_of_week, day in enumerate(week):
            if day != 0 and day_of_week < 5:  # Monday to Friday (0-4)
                business_days.append(day)
        
        # Only add week if there are business days
        if business_days:
            start_day = business_days[0]
            end_day = business_days[-1]
...
'''
    ...
    for week_num, start_day, end_day in weeks:
        ...
        html += f'''                    <div class="week-row">
                        <div class="week-label">{month}/{start_day}-{month}/{end_day}</div>
                        <ul class="week-tasks">
'''
        ...
        * {
        ...
        @media print {
            ...
            body {
                ...
                background: white; /* 底色留白，不印大面積背景漸層以防全頁黑掉，但保留區塊顏色 */
                padding: 0;
            }
            ...
            .grid-container {
                display: grid;
                grid-template-columns: 1fr 1fr; /* 分成平均兩欄 */
                gap: 15px;
            }

            .column:nth-child(1) { /* 每日任務 */
                grid-column: 1 / 2;
            }

            .column.weekly-column { /* 每週任務 */
                grid-column: 2 / 3;
            }

            .column:nth-child(3) { /* 每月任務 - 佔滿整排往下長 */
                grid-column: 1 / 3; 
            }
